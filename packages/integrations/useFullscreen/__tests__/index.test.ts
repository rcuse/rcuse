...
type EventKeys = KeysOfUnion<typeof events>

function setup(target: BasicTarget, options?: UseFullscreenOptions) {
...
describe('useFullscreen', () => {
  beforeEach(() => {
    ...
    vi.spyOn(document, 'addEventListener').mockImplementation((eventName, callback) => {
      if (events[eventName as EventKeys])
        events[eventName as EventKeys].add(callback)
    })

    vi.spyOn(document, 'removeEventListener').mockImplementation((eventName, callback) => {
      if (events[eventName as EventKeys])
        events[eventName as EventKeys].delete(callback)
    })
  })
